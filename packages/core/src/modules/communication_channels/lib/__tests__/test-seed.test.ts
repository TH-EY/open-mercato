...
describe('test-seed email capture scoping', () => {
  ...
  it('returns only exact-organization messages without privileged capture options', async () => {
    await seedCapture([
    ...
    expect(messages.map((message) => message.metadata?.subject)).toEqual(['exact'])
  })

  it('returns a tenant-wide message only with matching recipient and correlation token', async () => {
    await seedCapture([
      { ...makeCapturedMessage({ tenantId, organizationId: tenantId }, 'target@example.test', 'tenant-wide'), captureCorrelationToken: correlationToken },
      { ...makeCapturedMessage({ tenantId, organizationId: tenantId }, 'other@example.test', 'other-tenant-wide'), captureCorrelationToken: correlationToken },
    ])
    ...
    expect(messages.map((message) => message.metadata?.subject)).toEqual(['tenant-wide'])
  })
