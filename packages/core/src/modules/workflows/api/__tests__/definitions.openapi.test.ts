...
type RouteDocs = {
  methods: Record<string, { responses: DocumentedResponse[] }>
}
...
const routeDocs = [
  {
    ...
    docs: definitionsOpenApi as unknown as RouteDocs,
  },
  {
    path: '/workflows/definitions/[id]',
    handlers: { GET: noopHandler, PUT: noopHandler, DELETE: noopHandler },
    docs: definitionDetailOpenApi as unknown as RouteDocs,
  },
  {
    path: '/workflows/definitions/[id]/customize',
    handlers: { POST: noopHandler },
    docs: customizeOpenApi as unknown as RouteDocs,
  },
  {
    path: '/workflows/definitions/[id]/reset-to-code',
    handlers: { POST: noopHandler },
    docs: resetToCodeOpenApi as unknown as RouteDocs,
  },
]

const documentedResponses = routeDocs.flatMap(({ path, docs }) =>
  Object.entries(docs.methods).flatMap(([method, operation]) =>
...
describe('Workflow Definitions OpenAPI response schemas', () => {
  ...
  it.each(documentedResponses)(
    'declares and exemplifies $method $path response $response.status',
    ({ path, method, response }) => {
      const schema = schemaFor(path, method, String(response.status))
      expect(schema.description).not.toBe('Schema not declared')
      expect(response.schema.safeParse(response.example).success).toBe(true)
    }
  )
})
