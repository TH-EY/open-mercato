...
describe('sync_excel customers adapter', () => {
  ...
  it('emits smaller heartbeat batches within the configured logical batch size', async () => {
    mockReadSyncExcelUploadBuffer.mockResolvedValue(Buffer.from([
      'Record Id,First Name,Last Name,Lead Name,Email',
      'ext-1,Ada,Lovelace,Ada Lovelace,ada@example.com',
      'ext-2,Grace,Hopper,Grace Hopper,grace@example.com',
      'ext-3,Alan,Turing,Alan Turing,alan@example.com',
      'ext-4,Barbara,Liskov,Barbara Liskov,barbara@example.com',
      'ext-5,Donald,Knuth,Donald Knuth,donald@example.com',
      'ext-6,Linus,Torvalds,Linus Torvalds,linus@example.com',
    ].join('\n')))
    ...
    expect(batches[0]).toMatchObject({
      processedCount: 5,
      hasMore: true,
      cursor: expect.stringContaining('"offset":5'),
    })
    expect(batches[1]).toMatchObject({
      processedCount: 1,
      hasMore: false,
      cursor: expect.stringContaining('"offset":6'),
    })
  })
