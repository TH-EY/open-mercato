...
export async function run(argv = process.argv) {
  ...
  if (first === 'init') {
    ...
    try {
      ...
      const redactInitCredentials =
        parseBooleanToken(process.env.OM_INIT_REDACT_CREDENTIAL_OUTPUT ?? '') === true
      ...
      for (const entry of createdUsers) {
        ...
        if (redactInitCredentials) {
          console.log('║       Password: [redacted; see operator handoff]       ║')
        } else {
