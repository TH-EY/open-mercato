...
function isEmailTestMode(): boolean {
  return process.env.OM_TEST_MODE === 'true' || process.env.OM_TEST_MODE === '1'
}
...
function collectEmailText(node: unknown, parts: string[] = []): string[] {
  ...
  if (React.isValidElement(node)) collectEmailText(readElementProps(node).children, parts)
  return parts
}

async function captureEmailForTests(options: SendEmailOptions): Promise<void> {
  if (!isEmailTestMode()) return
