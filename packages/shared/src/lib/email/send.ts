...
async function captureEmailForTests(options: SendEmailOptions, from: string | null): Promise<void> {
  if (process.env.OM_TEST_MODE !== '1' && process.env.OM_TEST_MODE !== 'true') return
  ...
  const record: CapturedEmail = {
    ...
    text: (options.text || collectEmailText(options.react).join(' ')).replace(/\s+/g, ' ').trim(),
...
export async function sendEmail(options: SendEmailOptions): Promise<void> {
  ...
  await captureEmailForTests(options, fromAddr ?? null)
