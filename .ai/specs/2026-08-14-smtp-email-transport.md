...
- The existing SES transport establishes the selector contract before this pending work: unset or `resend` uses Resend, `ses` uses AWS SES, and unknown values fail closed. SMTP implementation must extend that accepted set additively with `smtp`; it must not add auto-detection or warning/fallback semantics.
...
- SMTP-specific recipient policy remains out of scope. The facade now has an independent, explicit `EMAIL_DELIVERY_POLICY=restricted` single-recipient Redis gate for tightly bounded SES deployments; selecting SMTP does not enable or bypass that SES-only gate.
...
Keep `sendEmail()` as the stable façade owning all cross-cutting behavior — the `OM_DISABLE_EMAIL_DELIVERY` / `OM_TEST_MODE` short-circuit and `resolveDefaultEmailFromAddress()` from-address resolution — and extend its explicit provider selection with an SMTP transport.

### Transport resolution

```
EMAIL_STRATEGY env ('resend' | 'ses' | 'smtp'; unknown value fails closed)
  → unset or 'resend' selects Resend
  → 'ses' selects the existing SES transport
  → 'smtp' selects the pending SMTP transport
```
...
| `packages/shared/src/lib/email/config.ts` | Extend the explicit strategy resolver additively with `smtp` and add zod-validated `resolveSmtpConfig()` including the TLS policy resolution. Unknown strategies remain fail-closed. |
...
|---------|----------------|---------|
| `EMAIL_STRATEGY` | `resend` \| `ses` \| `smtp`, unset by default | Unset or `resend` uses Resend; `ses` uses the existing AWS SES path; `smtp` uses this pending transport. Unknown values fail closed. |
| `SMTP_HOST` | string, required for smtp | SMTP server host used only after explicit `EMAIL_STRATEGY=smtp` selection. |
...
- **New `packages/shared/src/lib/email/__tests__/smtp.test.ts`** (`jest.mock('nodemailer')` with `createTransport` → `{ sendMail, close }`, mocked `@react-email/render`, env save/restore per the existing test's pattern):
  1. Resolution order — unset and explicit `resend` preserve Resend, explicit `ses` preserves the existing SES transport, explicit `smtp` selects SMTP, and unknown values fail closed.
...
|---|------|----------|---------------|------------|----------------|
| 1 | Behavior regression on the Resend path (all system email breaks) | High | Every email call site | Resend code moved verbatim into `transports/resend.ts`; existing test suite must pass unchanged; unset strategy remains Resend. | Low — dispatch layer is a thin switch. |
| 2 | Unintended transport flip from unrelated SMTP environment variables | Medium | Deployments with partial env | SMTP is selected only by explicit `EMAIL_STRATEGY=smtp`; `.env.example` documents the accepted strategies. | Low. |
