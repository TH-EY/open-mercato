...
- Project transaction amount, currency, status, and mode from the scoped transaction in the Deal API.
...
All transaction fields come from the same scoped `FinooAffiliateTransaction` row as the existing transaction ID and amount. Existing `affiliateProgramStatus` and attribution fields remain for compatibility.
