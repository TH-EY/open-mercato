...
- deletion, anonymization, legal-hold handling, notifications, or retention execution;
...
The MVP is the complete flagging loop described here: configure, preview, project, expire/reactivate, filter, reconcile, and operate it safely. A styled custom badge, data deletion/anonymization, legal holds, per-person exceptions, and a historical ledger are future work and are not prerequisites for deployment.
...
| Integration QA | Self-contained API/UI/queue coverage | Locally compliant | `TC-FINOO-RET-001` through `006` pass with isolated fixtures and teardown; deployed headed QA remains a release gate. |
