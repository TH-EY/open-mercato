...
The upgrade also provisions a dedicated credential for the pre-existing Finoo administrator because the same email exists in multiple tenants and an unscoped login can select the wrong tenant. A new Finoo-only Secrets Manager value is piped over stdin to `ensure-admin-credential`, which requires the exact Finoo tenant, organization, user ID, expected email, `--password-stdin`, and `--apply`. Its private system-only command is invoked directly by the maintenance CLI and is not registered in the generic command bus, so notification actions and generic command interceptors cannot dispatch or inspect its password input. It locks and revalidates that exact row inside the same transaction that hashes the password and revokes sessions; it no-ops when the value already matches, invalidates user access caches after a change, and uses `skipLog` so plaintext never enters command redo/audit storage. The smoke login includes the exact tenant ID in both the login URL and form body and verifies the `Finoo Superadmin` role. The reviewed helper is baked into the immutable application image; deployment extracts it from that digest and verifies the installed SHA-256, so a stale host checkout cannot weaken tenant scoping. Because the static login page can become reachable before the request-scoped authentication runtime is ready, deployment and rollback gates poll this authenticated smoke instead of treating `/login` alone as readiness. Each HTTP request has a five-second deadline, the secret read and container execution have independent hard deadlines, and failed login attempts are spaced by 65 seconds so the readiness probe cannot self-trigger the five-attempt, 60-second login rate limit. The six-attempt loop remains below the SSM controller budget. Existing bootstrap and smoke-role secrets are neither rotated nor modified by this delivery.
...
- Hardened staged deployment and rollback readiness after production staging demonstrated that `/login` can precede tenant-scoped authentication readiness. Authenticated smoke is now condition-polled with per-request, secret-read, and container-execution deadlines, with attempts spaced beyond the configured login-rate-limit window. The helper is extracted from the immutable image and hash-verified instead of being read from the mutable host checkout. Rollback verification removes any stale smoke helper, requires a successful fresh install, and cannot report success after a failed prerequisite hidden by Bash conditional `errexit` semantics.
