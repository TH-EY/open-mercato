...
### Operational hardening added after production feedback

To prevent `Job stale: no heartbeat for 60 seconds` failures during larger CSV imports:

- `sync_excel` now defaults to a smaller background batch size (`25` instead of `100`)
- the adapter emits heartbeat/progress checkpoints inside each logical batch (every 5 rows)
- per-row vector/fulltext fan-out is suppressed during `sync_excel` imports while query-index rows still update
- the `data_sync` engine enqueues one deferred fulltext + vector reindex for `customers:customer_person_profile` after the import completes
- Redis infra now pins `maxmemory-policy=noeviction` for the ElastiCache parameter group
