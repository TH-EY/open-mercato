...
NAME_PREFIX="${NAME_PREFIX:-openmercato-upstream-baseline-dokploy}"
SSM_STEP_NAME="${SSM_STEP_NAME:-Open Mercato deploy step}"
...
INSTANCE_ID="${INSTANCE_ID:-${PREVIEW_INSTANCE_ID:-}}"
...
{
  echo "bash <<'EOF_OM_SSM_STEP'"
  echo "set -euo pipefail"
  echo "${REMOTE_BODY}"
  echo "EOF_OM_SSM_STEP"
} > "${REMOTE_SCRIPT}"
