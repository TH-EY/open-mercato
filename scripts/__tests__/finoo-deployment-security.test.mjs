...
test('Finoo provisions and upgrades SES through the instance role without static AWS credentials', () => {
  ...
  assert.match(upgradeScript, /yarn mercato channel_ses assert-env-preset-absent/)
  assert.match(upgradeScript, /yarn mercato channel_ses remove-env-preset/)
  assert.match(upgradeScript, /ses_preset_seeded=true/)
  assert.match(upgradeScript, /remove_seeded_ses_preset "\$active_container" "\$candidate_container"/)
  assert.match(upgradeScript, /\.State\.Running/)
