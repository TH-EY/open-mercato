...
describe('apps/mercato search indexing headers', () => {
  it('marks only the Finoo demo host as non-indexable', async () => {
    if (!nextConfig.headers) {
      throw new Error('[internal] next.config headers are not configured')
    }

    const rules = await nextConfig.headers()
    const hostname = 'finoo.om.they.dev'
    ...
    expect(new RegExp(`^${hostPattern}$`).test('finooXomYtheyZdev')).toBe(false)
  })
})
