...
function peselIdentifier(runId: string, index: number): string {
  const day = String((index % 28) + 1).padStart(2, '0')
  const serial = numericIdentifier(runId, index, 4)
  const firstTenDigits = `9001${day}${serial}`
  const weights = [1, 3, 7, 9, 1, 3, 7, 9, 1, 3]
  const weightedSum = [...firstTenDigits].reduce(
    (sum, digit, digitIndex) => sum + Number(digit) * weights[digitIndex]!,
    0,
  )
  return `${firstTenDigits}${(10 - (weightedSum % 10)) % 10}`
}
...
function buildScenario(runId: string, options: ScenarioOptions): ApplyFormScenario {
  ...
  const pesel = peselIdentifier(runId, options.index)
