...
function payload(commissionMode: 'percentage' | 'fixed' | null, transaction = false) {
  const transactionProjection = transaction ? {
    ...
    commissionMode: commissionMode ?? 'legacy_deal_amount' as const,
  ...
  return {
    ...
    attribution: transaction ? {
      ...
      affiliateTransactionId: transactionProjection!.id,
...
describe('DealAttributionWidget', () => {
  ...
  it('keeps the canonical transaction visible after its attribution is removed', async () => {
    const editorPayload = payload('fixed', true)
    renderWidget({ ...editorPayload, attribution: null })
