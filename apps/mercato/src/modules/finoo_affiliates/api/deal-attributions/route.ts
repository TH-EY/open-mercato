...
export async function GET(request: Request): Promise<Response> {
  ...
  const [attribution, transaction, affiliates, affiliateMemberships, statuses] = await Promise.all([
    ...
    findWithDecryption(
      em,
      FinooAffiliate,
      { ...resolved.scope, isActive: true, deletedAt: null },
      undefined,
      resolved.scope,
    ),
  ...
  const commissionModeByUserId = new Map(
    affiliateMemberships.flatMap((affiliate) => (
      affiliate.customerUserId ? [[affiliate.customerUserId, affiliate.commissionMode ?? null] as const] : []
    )),
  )
  return NextResponse.json({
    ...
    affiliates: affiliates.map((user) => ({
      id: user.id,
      displayName: user.displayName,
      email: user.email,
      commissionMode: commissionModeByUserId.get(user.id) ?? null,
    })),
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    GET: {
      ...
      responses: [{ status: 200, description: 'Deal attribution editor data', schema: z.object({ attribution: attributionSchema.nullable(), affiliates: z.array(z.object({ id: z.string().uuid(), displayName: z.string(), email: z.string().email(), commissionMode: z.enum(['percentage', 'fixed']).nullable() })), statuses: z.array(z.object({ id: z.string().uuid(), value: z.string(), label: z.string() })) }) }],
    },
    PUT: {
